...
DEFAULT_PROMPTS: dict[str, str] = {
    "analyze_clarify_and_answer": (
        ...
        "   - HỆ THỐNG CÓ HAI MÃ CHIẾN DỊCH KHÁC NHAU, THUỘC HAI BẢNG KHÁC NHAU - TUYỆT ĐỐI không dùng lẫn cho nhau:\n"
        "     + `campaign_ai_id`: mã chiến dịch dùng cho bảng `report_offer_detail_isdn` - đây là bảng NGUỒN CHÍNH cho mọi câu hỏi về "
        "tỉ lệ gửi tin thành công, tỉ lệ nhận tin thành công, và tỷ lệ phản hồi (xem chi tiết ở Business Knowledge). Ưu tiên slot này cho các câu hỏi dạng đó.\n"
        "     + `campaign_id`: mã chiến dịch dùng cho các bảng log cũ `webservice_log_v2`/`sms_log_v2` (không còn là nguồn chính cho tỉ lệ gửi/nhận/phản hồi nữa).\n"
        "     + Cả hai đều LUÔN LUÔN LÀ SỐ. Hệ thống HIỆN KHÔNG có bảng tra cứu tên/nhãn chiến dịch sang mã số. "
        "Vì vậy nếu người dùng chỉ nói TÊN hoặc NHÃN chiến dịch (ví dụ 'chiến dịch 5G', 'chiến dịch DATA') mà KHÔNG cho con số cụ thể, "
        "đó KHÔNG ĐỦ để điền `campaign_ai_id`/`campaign_id` - phải coi là còn thiếu thông tin và hỏi lại người dùng đúng con số, "
        "TUYỆT ĐỐI KHÔNG được tự suy đoán/gán nhãn chữ (như '5G') vào các slot này.\n"
        "   - `action_channel` (cột trong `report_offer_detail_isdn`): biểu thị LOẠI chiến dịch ('SMS' = mời gói, 'notify' = click link) - "
        "ĐÂY LÀ KHÁI NIỆM HOÀN TOÀN KHÁC với slot `channel` bên dưới (kênh gửi tin SMS/MyViettel/Callbot), dù trùng chữ 'SMS'. "
        "Xem chi tiết cách phân biệt và dùng đúng trong Business Knowledge.\n"
        "   - Lưu ý: cột `program_code`, `ten_usecase` trong `f_adpm_aimkt_campaign_customer_detail` KHÔNG liên quan đến việc định danh chiến dịch - "
        "TUYỆT ĐỐI không dùng `program_code`/`ten_usecase` để lọc hay xác định chiến dịch theo `campaign_id`/`campaign_ai_id`.\n"
        "   - Các slot khác: khoảng thời gian (`time_range`), kênh truyền thông (`channel` như SMS, MYVIETTEL, CALLBOT - chỉ dùng cho câu hỏi liên quan webservice_log_v2/sms_log_v2, KHÔNG phải action_channel), "
